...
- Installer releases stay aligned with the bundled Unraid OS version. The release-lock merge supplies `Release-As: <OS-version>`; Release Please then owns that exact `Installer-<version>` tag and GitHub Release. After it creates a release, the same workflow builds the exact released SHA and attaches the validated assets.
...
- Installer-only code changes wait for the next Unraid OS release instead of advancing an independent installer version.
