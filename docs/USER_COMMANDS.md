...
- Installer releases keep the bundled Unraid OS version as their base. Installer-only releases use Release Please service-pack revisions such as `Installer-7.3.2-sp.1`; the next OS release-lock merge supplies `Release-As: 7.3.3` and returns to the exact OS version. Release Please owns the tag and GitHub Release, then the same workflow builds the exact released SHA and attaches the validated assets.
- Manual publication is retry-only: it requires an existing Release Please release and a successful build run whose source SHA matches that release tag.
...
- Installer-only changes advance the service-pack revision without changing the bundled OS base version.
...
- After publishing the latest stable bundled image, the release publisher removes bundled IMG assets from older stable `Installer-*` releases. Those releases keep their online installer assets when present.
